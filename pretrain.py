...
def eval(args, dataloader, model, matcher, netG, netD, criterionBCE, criterion_matcher):
    ...
    with torch.no_grad():
        ...
        for data, jig_l, class_l, data_non_mask, data64_non_mask, cl_data_mask, _ in tqdm(dataloader,
            ...
            class_correct = class_correct+torch.sum(cls_pred1 == class_l1.data)
            class_correct = class_correct+torch.sum(cls_pred2 == class_l2.data)
            class_correct = class_correct+torch.sum(cls_pred3 == class_l3.data)

            class_correct_1 = class_correct_1+torch.sum(cls_pred1 == class_l1.data)
            class_correct_2 = class_correct_2+torch.sum(cls_pred2 == class_l2.data)
            class_correct_3 = class_correct_3+torch.sum(cls_pred3 == class_l3.data)

            jigsaw_correct = jigsaw_correct+torch.sum(jig_pred == jig_l.data)
            ...
            if args.constractive_lambda != 0:
                ...
                returnData["ConstractiveLoss"] = returnData["ConstractiveLoss"]+constractive_loss.item() / total
            ...
            if args.matcher_lambda != 0:
                ...
                returnData["ReasonabilityLoss"] = returnData["ReasonabilityLoss"]+reasonability_loss.item() / total
            ...
            if args.is_recover_training == 1:
                ...
                returnData["RecoverLoss"] = returnData["RecoverLoss"]+(errD.item() + errG.item()) / 2 / total
...
def main(args):
    ...
    if len(device_ids) > 1:
        ...
        model = torch.nn.DataParallel(model, device_ids=device_ids,broadcast_buffers=False)
        matcher = torch.nn.DataParallel(matcher, device_ids=device_ids,broadcast_buffers=False)
        netG = torch.nn.DataParallel(netG, device_ids=device_ids,broadcast_buffers=False)
        netD = torch.nn.DataParallel(netD, device_ids=device_ids,broadcast_buffers=False)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        with tqdm(total=len(train_dataloader)) as t:
            for i, (
                ...
                if args.constractive_lambda != 0:
                    constractive_loss = (hidden_feat_non_mask - hidden_feat_mask).pow(2).sum(axis=1).sqrt().mean()
                    AvgConstractiveLoss = AvgConstractiveLoss+constractive_loss.item() / len(train_dataloader)
                ...
                if args.matcher_lambda != 0:
                    ...
                    reasonability_loss = criterion_matcher(out_cls_false, y_out_cls_false) \
                                         + criterion_matcher(out_cls_true, y_out_cls_true)
                    AvgReasonabilityLoss = AvgReasonabilityLoss+reasonability_loss.item() / len(train_dataloader)
                ...
                if args.is_recover_training == 1:
                    ...
                    AvgRecoverLoss = AvgRecoverLoss+(errD.item() + errG.item()) / 2 / len(train_dataloader)
                ...
                AvgJigLoss = AvgJigLoss+Jig_loss.item() / len(train_dataloader)
                AvgClassLoss1 = AvgClassLoss1+class_loss1.item() / len(train_dataloader)
                AvgClassLoss2 = AvgClassLoss2+class_loss2.item() / len(train_dataloader)
                AvgClassLoss3 = AvgClassLoss3+class_loss3.item() / len(train_dataloader)
                AvgClassLoss = AvgClassLossz=class_loss.item() / len(train_dataloader)
                AvgTotalLoss = AvgTotalLoss+loss.item() / len(train_dataloader)
