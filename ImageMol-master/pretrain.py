...
# evaluation评估函数：评估拼图任务的准确性、不同数量的聚类任务的准确性、对比损失、合理性损失
"""
...
def main(args):
    ...
    if len(device_ids) > 1:
        ...
        #有4个GPU，并且你只想在前两个上运行模型，你可以设置device_ids=[0,1]
    ...
    train_dataset = ImageDataset(name_train, labels_train, img_transformer=transforms.Compose(img_tra),
